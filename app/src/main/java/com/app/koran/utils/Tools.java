...
import com.app.koran.ActivityWebView;
...
public class Tools {
    ...
    public static void directLinkToBrowser(AppCompatActivity activity, String url) {
        try {
            if (AppConfig.OPEN_IN_APP_BROWSER) {
                ActivityWebView.navigate(activity, url);
            } else {
    ...
    public static void displayImageThumbnailPage(Context ctx, Page p, ImageView imageView) {
        Post post = Tools.getPostDataFromPage(p);
        Tools.displayImageThumbnail(ctx, post, imageView);
    }

    private static Post getPostDataFromPage(Page p) {
        Post post = new Post();
        post.thumbnail_images = p.thumbnail_images;
        post.thumbnail = p.thumbnail;
        post.attachments = p.attachments;
        return post;
    }

    public static String getPageThumbnailUrl(Page p) {
        Post post = Tools.getPostDataFromPage(p);
        return Tools.getPostThumbnailUrl(post);
    }

    public static String getPostThumbnailUrl(Post p) {
        String url = "";
        if (p.thumbnail_images != null) {
            if (p.thumbnail_images.full != null && p.thumbnail_images.full.url != null && !p.thumbnail_images.full.url.equals("")) {
                url = p.thumbnail_images.full.url;
            } else if (p.thumbnail_images.medium != null && p.thumbnail_images.medium.url != null && !p.thumbnail_images.medium.url.equals("")) {
                url = p.thumbnail_images.medium.url;
            } else if (p.thumbnail_images.medium_large != null && p.thumbnail_images.medium_large.url != null && !p.thumbnail_images.medium_large.url.equals("")) {
                url = p.thumbnail_images.medium_large.url;
            } else if (p.thumbnail_images.large != null && p.thumbnail_images.large.url != null && !p.thumbnail_images.large.url.equals("")) {
                url = p.thumbnail_images.large.url;
            }
        }
    ...
    public static List<Page> getSortedPageById(List<Page> pages) {
        if (pages != null && pages.size() > 0) {
            Collections.sort(pages, new Comparator<Page>() {
                @Override
                public int compare(final Page p1, final Page p2) {
                    return Float.compare(p2.id, p1.id);
                }
            });
        }
        return pages;
    ...
    public static boolean isUrlFromImageType(String url) {
        try {
